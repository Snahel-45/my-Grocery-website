    ...
    <header className={`bg-white fixed top-0 right-0 left-0 z-50 ${isScrolled ? 'drop-shadow-[0_4px_25px_rgba(0,0,0,0.1)]' : ""}`}>
        <nav className='max-w-[1400px] mx-auto px-10 md:h-[14vh] h-[12vh] flex justify-between items-center'>
            <Link to="/" className='text-3xl font-bold'>
                Grocy<span className='text-orange-500'>Time</span>
            </Link>
            ...
            <div className='md:flex p-1 border-2 border-orange-500 rounded-full hidden'>
                ...
                {/* Hamburger */}
                <button href="" className='text-zinc-800 text-3xl md:hidden' onClick={toggleMenu}>
                    {showMenu ? <TbMenu3/> : <TbMenu2/>}
                </button>
            </div>
            {/* Mobile Menu */}
              <ul  className={`flex flex-col gap-y-12 bg-orange-500/15 p-10 backdrop-blur-xl 
                     rounded-xl items-center shadow-xl md:hidden absolute top-28 transform -translate-x-1/2 
